...
router.put('/profile', auth, [
  ...
  try {
    ...
    updateQuery += updateEntries.map(([key, value]) => `${key} = ${value}`).join(', ');
    updateQuery += ', updatedAt = GETDATE() OUTPUT INSERTED.id, INSERTED.email, INSERTED.firstName, INSERTED.lastName, INSERTED.role WHERE id = ${req.user.id}';
